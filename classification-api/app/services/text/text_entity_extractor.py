...
class TextEntityExtractor:
    ...
    def __get_text_nominative_form(self, text):
        ...
        for token in doc:
            ...
            # Check if the word is an adjective modifying a noun
            if token.pos_ == "ADJ" and token.head.pos_ == "NOUN":
                noun_parsed = self.morph.parse(token.head.text)[0]

                # Filter out any None values before inflecting
                ...
                # Default to the normal form if not an adjective-noun alignment
