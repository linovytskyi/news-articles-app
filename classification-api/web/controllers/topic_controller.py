...
from web.dependencies import get_topic_service
from web.schemas.topic import Topic, TopicKeyword
...
def get_add_topics(topic_service: TopicRepository = Depends(get_topic_service)):
    logger.info("Received request to get all topics")
    all_topics = topic_service.get_all_topics()
    return all_topics

@router.get("/{topic_id}", response_model=Topic)
def get_topic_by_id(topic_id: int, topic_service: TopicRepository = Depends(get_topic_service)):
...
def get_topic_by_name(name: str, topic_service: TopicRepository = Depends(get_topic_service)):
